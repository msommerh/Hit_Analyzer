...
def MANtag_pT_CSV_from_single_file(input_file, MC=True, BG=False):
	MANtag, pT, CSV, deep_CSV= MANtag_pT_CSV_from_file(input_file, MC=MC, BG=BG)
	return np.array(MANtag), np.array(pT), np.array(CSV), np.array(deep_CSV)

def MANtag_pT_CSV_from_multiple_files(file_list, MC=True, BG=False):
	MANtag, pT, CSV, deep_CSV = [], [], [], []
	for f in file_list:
		_MANtag, _pT, _CSV, _deep_CSV = MANtag_pT_CSV_from_file(f, MC=MC, BG=BG)
		MANtag += _MANtag
		pT += _pT
		CSV += _CSV
		deep_CSV += _deep_CSV
...
def AnalysisStep0_LoadData(signal_files, bg_files):
	signal_MANtag_data, signal_pT_data, signal_CSV_data, signal_deep_CSV_data = MANtag_pT_CSV_from_multiple_files(signal_files, MC=True, BG=False)
        bg_MANtag_data, bg_pT_data, bg_CSV_data, bg_deep_CSV_data = MANtag_pT_CSV_from_multiple_files(bg_files, MC=True, BG=True)
	return (signal_MANtag_data, signal_pT_data, signal_CSV_data, signal_deep_CSV_data), (bg_MANtag_data, bg_pT_data, bg_CSV_data, bg_deep_CSV_data)

def AnalysisStep1_DeriveCuts(cut_bins, bg_data):
        bg_MANtag_data, bg_pT_data, bg_CSV_data, bg_deep_CSV_data = bg_data
...
def AnalysisStep2_GeneratePlots(cut_bins, signal_data, bg_data, MANtag_cuts, CSV_cuts, deep_CSV_cuts):
	signal_MANtag_data, signal_pT_data, signal_CSV_data, signal_deep_CSV_data = signal_data
	bg_MANtag_data, bg_pT_data, bg_CSV_data, bg_deep_CSV_data = bg_data
	ANN_binned_tagged_jets_hist([(signal_MANtag_data, signal_pT_data, signal_CSV_data, signal_deep_CSV_data, "signal_large", (200,3000))], MANtag_cuts, CSV_cuts, deep_CSV_cuts, cut_bins, 60)
	ANN_binned_tagged_jets_hist([(bg_MANtag_data, bg_pT_data, bg_CSV_data, bg_deep_CSV_data, "BG_large", (200,3000))], MANtag_cuts, CSV_cuts, deep_CSV_cuts, cut_bins, 60)
	...
	signal_deep_CSV_hist = signal_tagged_jets_file.Get('signal_large_deep_CSV')
	...
	BG_deep_CSV_hist = BG_tagged_jets_file.Get('BG_large_deep_CSV')
	...
	Efficiency_vs_pT("signal_large",[(signal_ANN_hist, "MANtag", 3), (signal_CSV_hist, "CSV", 4), (signal_deep_CSV_hist, "deep_CSV", 6)], signal_AllJets_hist,1, legend_shift=True, BG=False)
	Efficiency_vs_pT("BG_large",[(BG_ANN_hist, "MANtag", 3), (BG_CSV_hist, "CSV", 4), (BG_deep_CSV_hist, "deep_CSV", 6)], BG_AllJets_hist,0.3,BG=True)
...
if __name__ == "__main__":
	...
	signal_path = "/afs/cern.ch/work/m/msommerh/public/HitAnalyzer4/{}/M{}/flatTuple_{}.root"
        bg_path = "/afs/cern.ch/work/m/msommerh/public/HitAnalyzer4/QCD/{}/flatTuple_{}.root"
	...
	AnalysisStep2_GeneratePlots(cut_bins, signal_data, bg_data, MANtag_cuts, CSV_cuts, deep_CSV_cuts)
