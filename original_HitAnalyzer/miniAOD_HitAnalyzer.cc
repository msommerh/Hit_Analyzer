...
//  edm::EDGetTokenT< reco::PFMETCollection >		  METToken;   //deactivated to run on miniAOD
...
//  METToken	 = consumes<reco::PFMETCollection		> (edm::InputTag(labelMET));   //deactivated to run on miniAOD
  ...
  HitAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 // Handle<reco::PFMETCollection		      > METs	 ; iEvent.getByToken( METToken	   , METs     );  //deactivated to run on miniAOD
...
//  MET_over_sumEt = (METs->front()).et()/(METs->front()).sumEt();  //deactivated to run on miniAOD
  ...
  //event selection
//  if (MET_over_sumEt>=MET_over_sumEt_cut_) return;    //deactivated to run on miniAOD
